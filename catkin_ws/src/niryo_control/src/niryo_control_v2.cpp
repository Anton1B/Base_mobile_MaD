...
int main(int argc, char **argv)
{
	...
	while (true)
	{
		...
		case 1:
			...
			ask_niryo.data = 10; //Demande à actions_niryo de lancer l'action "servir"
		...
		case 4:
			...
			ask_niryo.data = 11; //Demande à actions_niryo de lancer l'action "ranger"
			niryo_pub.publish(ask_niryo);
		...
		case 6:
			...
			ask_niryo.data = 99; //Demande d'observation
			niryo_pub.publish(ask_niryo);
			ros::spinOnce();

			std::cout << " \n Objectif envoye\n";
			message.data = "Observation manuelle en cours. Utilisez les flèches pour déplacer la caméra";
			client_pub.publish(message);
